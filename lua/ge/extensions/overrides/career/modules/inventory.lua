...
-- Make sure AIWorkerManager is loaded and state is reloaded for the current save slot
local AIWorkerManager_ok, AIWorkerManager = false, nil

local function ensureAiStateLoaded()
...
local aiNotifyCooldown = 0

local function resolveVehicleType(jbeam)
  -- Manual map first
  local vehicleTypeMap = {
    atv = "car", autobello = "car", barstow = "coupe", bastion = "coupe", bluebuck = "sedan",
    bolide = "sportscar", burnside = "sedan", bx = "coupe", caravan = "trailer", cargotrailer = "trailer",
    citybus = "truck", containertrailer = "trailer", covet = "hatchback", dolly = "trailer", dryvan = "van",
    dumptruck = "semi", etk800 = "sedan", etkc = "sportscar", etki = "sedan", flatbed = "trailer",
    hopper = "car", lansdale = "van", legran = "sedan", log_trailer = "trailer", md_series = "semi",
    mirmar = "coupe", moonhawk = "coupe", nine = "coupe", pessima = "sedan", pickup = "truck",
    pigeon = "truck", racetruck = "truck", roamer = "truck", sbr = "sportscar", scintilla = "sportscar",
    sunburst2 = "coupe", tanker = "trailer", tiltdeck = "trailer", us_semi = "semi", utv = "car",
    van = "van", vivace = "hatchback", wendover = "coupe", wigeon = "car", wl40 = "semi",
    -- ... you can add new mappings here if needed ...
  }
  jbeam = tostring(jbeam):lower()
  if vehicleTypeMap[jbeam] then
    return vehicleTypeMap[jbeam]
  end

  local core_vehicles = core_vehicles or require("core/core_vehicles")
  local modelInfo = core_vehicles.getModel(jbeam)
  if modelInfo then
    -- Try category
    if modelInfo.category and type(modelInfo.category) == "string" then
      local c = modelInfo.category:lower()
      if c:find("bus") then return "truck"
      elseif c:find("semi") then return "semi"
      elseif c:find("trailer") then return "trailer"
      elseif c:find("van") then return "van"
      elseif c:find("pickup") or c:find("truck") then return "truck"
      elseif c:find("sport") then return "sportscar"
      elseif c:find("coupe") then return "coupe"
      elseif c:find("wagon") then return "wagon"
      elseif c:find("hatch") then return "hatchback"
      elseif c:find("sedan") then return "sedan"
      end
    end
    -- Try bodyType
    if modelInfo.bodyType and type(modelInfo.bodyType) == "string" then
      local b = modelInfo.bodyType:lower()
      if b:find("van") then return "van"
      elseif b:find("semi") then return "semi"
      elseif b:find("truck") then return "truck"
      elseif b:find("trailer") then return "trailer"
      elseif b:find("sedan") then return "sedan"
      elseif b:find("wagon") then return "wagon"
      elseif b:find("bus") then return "truck"
      end
    end
  end
  return "car" -- fallback for unknowns
end
...
local function onExtensionLoaded()
  ...
  local AIWM = require("career/modules/AIWorkerManager")
  AIWM.loadState()
  ...
  for inventoryId, v in pairs(vehicles) do
    local jbeam = (v.model or v.JBeam or ""):lower()
    local resolved = resolveVehicleType(jbeam)
    print("Post-load mapping: InventoryID", inventoryId, "JBeam", jbeam, "resolved to type", resolved)
    v.type = resolved
  end
...
local function onSaveCurrentSaveSlot(currentSavePath, oldSaveDate, vehiclesThumbnailUpdate)
  ...
  if AIWorkerManager_ok and AIWorkerManager and AIWorkerManager.saveState then
    AIWorkerManager.saveState()
  end
...
local function addVehicle(vehId, inventoryId, options)
  ...
  if vehicle and vehicleData then
    if not inventoryId then
      ...
      while vehicles[inventoryId] do
        ...
        if attempts > 10000 then
          log("E", "inventory", "INFINITE LOOP in addVehicle: Too many attempts to find empty id!")
          break
        end
      end
    end
    ...
    local modelName = vehicle.JBeam               -- JBeam name string
    local configName = vehicleData.config and vehicleData.config.partConfigFilename

    -- Prefer BeamNG config display name if available
    if modelName and configName then
      local _, configBase, _ = path.splitWithoutExt(configName)
      local baseConfig = core_vehicles.getConfig(modelName, configBase)
      if baseConfig and baseConfig.Name then
        displayName = baseConfig.Name
      end
    end

    -- Fallback: live vehicle info name
    if (not displayName or displayName == "") and vehicleData.vdata and vehicleData.vdata.information and vehicleData.vdata.information.name then
      displayName = vehicleData.vdata.information.name
    end

    -- Fallback: model/JBeam name
    if not displayName or displayName == "" then
      displayName = modelName or "Unknown Vehicle"
    end

    -- --- VEHICLE TYPE LOGIC ---
    local jbeam = (vehicle.JBeam or ""):lower()
    local type = vehicleTypeMap[jbeam] or "car"

    -- --- BUILD VEHICLE TABLE ---
    vehicles[inventoryId] = vehicles[inventoryId] or {}
    vehicles[inventoryId].model = modelName or ""
    vehicles[inventoryId].config = vehicleData.config
    vehicles[inventoryId].id = inventoryId
    vehicles[inventoryId].niceName = displayName                 -- always human-readable!
    vehicles[inventoryId].type = type                            -- ALWAYS from map!
    ...
    if modelName and vehicleData.config and vehicleData.config.partConfigFilename then
      local dir, configBase, ext = path.splitWithoutExt(vehicleData.config.partConfigFilename)
      local baseConfig = core_vehicles.getConfig(modelName, configBase)
    ...
    vehicle:queueLuaCommand(string.format(
      "if not partCondition.getConditions() then partCondition.initConditions() end obj:queueGameEngineLua('career_modules_inventory.updatePartConditions(%d, %d)')",
      vehId, inventoryId
    ))
...
local function removeVehicle(inventoryId)
  local invVeh = vehicles[inventoryId]
  local key1 = tostring(inventoryId)
  local key2 = (invVeh and invVeh.id) and tostring(invVeh.id) or nil
  if AIWorkerManager_ok and AIWorkerManager and AIWorkerManager.unassignWorkerFromVehicle then
    AIWorkerManager.unassignWorkerFromVehicle(key1)
    if key2 and key2 ~= key1 then
      AIWorkerManager.unassignWorkerFromVehicle(key2)
    end
  end
...
local function getVehicleUiData(inventoryId, inventoryIdsInGarage)
  ...
  -- Check both keys: inventory index and vehicleId string
  local invVeh = vehicles[inventoryId]
  local key1 = tostring(inventoryId)
  local key2 = (invVeh and invVeh.id) and tostring(invVeh.id) or nil

  local aiLocked = (aiAssignments[key1] ~= nil) or (key2 ~= nil and aiAssignments[key2] ~= nil)
  ...
  -- Final override: if locked by AI, block all relevant actions
...
local function onUpdate(dtReal, dtSim, dtRaw)
  ...
  aiNotifyCooldown = (aiNotifyCooldown or 0) + dtReal
  local AIWM = require("career/modules/AIWorkerManager")
  local totals = AIWM.tick(dtReal)

  if totals and totals.net and totals.net > 0 then
    -- Credit net to player
    if career_modules_playerAttributes and career_modules_playerAttributes.addAttributes then
      career_modules_playerAttributes.addAttributes(
        { money = totals.net },
        { tags = { "aiWorkerIncomePeriodic" }, label = string.format("AI payout: $%d", math.floor(totals.net)) },
        true
      )
    end

    -- Show toast no more than once every ~1 second (to avoid spam if multiple segments pay at once)
    if aiNotifyCooldown >= 1.0 and guihooks and guihooks.trigger then
      guihooks.trigger("toastrMsg", {
        type = "info",
        label = "aiHalfDayPayout",
        title = "Half‑day AI payout",
        msg = string.format(
          "Segments: %d\nGross: $%d\nPaid to workers: $%d\nNet: $%d",
          totals.segments or 1, math.floor(totals.gross or 0), math.floor(totals.wages or 0), math.floor(totals.net or 0)
        )
      })
      aiNotifyCooldown = 0
    end
    ...
    {
      {
		callback = function(inventoryId)
			...
			-- Check both key forms (inventory index and vehicleId string in your table)
			local invVeh = vehicles[inventoryId]
			local key1 = tostring(inventoryId)
			local key2 = (invVeh and invVeh.id) and tostring(invVeh.id) or nil

			if aiAssignments[key1] or (key2 and aiAssignments[key2]) then
				ui_message("Vehicle is currently assigned to an AI Worker and unavailable.", 6, "career")
				return
			end
	   ...
	   },
      {
		callback = function(inventoryId)
		  ...
		  spawnVehicleAndTeleportToGarage(false, inventoryId, true)
		end,
...
function career_modules_aiWorkerGetAssignmentsLive()
  local AI = require("career/modules/AIWorkerManager")
  local out = {}
  local amap = AI.getAssignments() or {}
  for vid, data in pairs(amap) do
    if data and data.worker then
      local allowed, minutes = AI.isRecallAllowed(vid)
      local remainSec, nextLabel = 0, ""
      if AI.getNextPayoutEta then
        remainSec = AI.getNextPayoutEta(vid) or 0
        local mm = math.floor(remainSec / 60)
        local ss = remainSec % 60
        nextLabel = string.format("%dm %02ds", mm, ss)
      end
	  
	  local lookupId = data.vehicleId or tonumber(vid)

	  local invVeh = vehicles[lookupId]
	  local displayName = invVeh and invVeh.niceName or data.vehicleName or data.vehicleType or tostring(lookupId)
	  print("Assignment vehicle:", lookupId, "name:", displayName)	
	
      out[#out+1] = {
        vehicleId = tonumber(vid),
        vehicleName   = displayName,
        worker        = data.worker,
        recallAllowed = allowed,
        timeLeftMin   = minutes or 0,
        nextPayoutSec = remainSec,
        nextPayout    = nextLabel
      }
    end
  end
  table.sort(out, function(a, b) return tostring(a.vehicleId) < tostring(b.vehicleId) end)
  return out
end
...
function M.getVehicleDiagnosticsUiData(vehicleId)
  ...
  local count, sum = 0, 0
  local damagedParts = {}
  for partName, cond in pairs(partConditions) do
    if cond.condition then
      sum = sum + cond.condition
      count = count + 1
      if cond.condition < 0.98 then
        local pcent = math.floor((cond.condition or 0) * 100)
        table.insert(damagedParts, {name=partName, percent=pcent})
      end
    end
  end
  ...
  for _, p in ipairs(damagedParts) do
    if p.percent <= 85 then needsRepair = true end
  end
  return {
    vehicleId    = vehicleId,
    vehicleName  = vehicle.niceName or vehicle.model or '-',
...
M.getVehicleDiagnosticsUiData = M.getVehicleDiagnosticsUiData or getVehicleDiagnosticsUiData

M.getDiagnosticsReport = getDiagnosticsReport
...
_G.career_modules_aiWorkerGetAssignmentsLive = career_modules_aiWorkerGetAssignmentsLive
